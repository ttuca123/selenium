...

{
	...
	public void setUp() throws ClassNotFoundException, SQLException, IOException,
			MessagingException, TestLinkAPIException

	{
...

		{
			...
			servicoPadrao.getResultado()
					.preencherErro(e.getCause(), EnumTipoErro.TEA);

		}
		catch (NoSuchElementException elementException)
		{

			servicoPadrao.getResultado().preencherErro(elementException.getCause(),
					EnumTipoErro.LOCATION);

		}
		catch (org.openqa.selenium.TimeoutException timeException)
		{

			servicoPadrao.getResultado().preencherErro(timeException.getCause(),
					EnumTipoErro.TIMEOUT);
			...
			IOException, MessagingException
	{
		...
		if ( servicoPadrao.isExecutarSolicitacao() )
		{
			...
			catch (AssertionError erroAssert)
			{
				...
				servicoPadrao.getResultado().preencherErro(erroAssert.getCause(),
						EnumTipoErro.REGRA_NEGOCIO);
		...
		if ( servicoPadrao.isExigeBaixa() )
		{
			...
			for (EnumServico service : servicoPadrao.getServicos())
			{
						...
						.equals("SOLICITAÇÃO CONCLUÍDA") )
				{
					...
					metodoInvocado = classe2.getMethod("preencherDadosBaixa", params);
			...
			ClassNotFoundException, SQLException
	{
		...
		EnumTipoExecucao tipoExecucao = servicoPadrao.getResultado().getProjeto()
				.getExecucao();
		...
		if ( tipoExecucao.equals(EnumTipoExecucao.TODOS)
				|| (ConnectJDBC.verificarStatusExecucao(
						...
						servicoPadrao.getResultado().getProjeto()).getTipoexec()
						.equals(tipoExecucao)) )
		{
			...
			switch (servicoPadrao.getEnumTipoServico())
			{
				case IMOVEL:
					...
					if ( servicoPadrao.isGerarPar() )
					{
						...
						servicoPadrao.setQtdContratosDestino(ConnectJDBC
								.verificarQtdContratos(inscricoes.get(1).toString()));
				...
				case CLIENTE:
					...
					servicoPadrao.setCpfCnpj(ConnectJDBC.gerarCpfCnpj(servicoPadrao));

					servicoPadrao.setQtdContratoOrigem(ConnectJDBC
							.verificarQtdContratosCpfCnpj(servicoPadrao.getCpfCnpj()));
	...
	private void solicitarServico() throws Exception
	{
						...
						servicoPadrao.getServicos()) )
		{
			...
			atendimentoPage.cancelarServicosPendentes(servicoPadrao.getServicos());

		}

		if ( (servicoPadrao.getTiposPgtos().contains(
				EnumTipoPagamento.PARCELADO_COM_ENTRADA) || servicoPadrao
				.getTiposPgtos().contains(EnumTipoPagamento.PARCELADO_SEM_ENTRADA)) )
		{
		...
		if ( servicoPadrao.isExecutarSolicitacao() )
		{
			...
			itemAtendimentoPage = PageFactory.initElements(DriverUtils.getDriver(),
					ItemAtendimentoPage.class);
			...
			if ( servicoPadrao.isExecutarSolicitacao() )
			{
...

				{
					...
					atendimentoPage.confirmarServicoCasado(servicoPadrao.getTiposPgtos()
							.get(1), servicoPadrao.getValidacao());
				...
				if ( servicoPadrao.isExecutarSolicitacao() )
				{
					...
					servicoPadrao.getValidacao().validarTipoPagamento(servicoPadrao);
