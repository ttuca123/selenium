...
public class ServicoPadrao
{
	...
	protected static final String NUMERO_LACRE_LIGACAO = Util.gerarNumero(99999)
			.toString();
		...
		for (Annotation anotacao : classe.getDeclaredAnnotations())
		{
					...
					&& (anotacao instanceof CasoDeTesteServico) )
			{
				...
				casoTeste = testCase
						...
						.concat("-SRV-")
						.concat(
								StringUtils.leftPad(testCase.servico().getCodigo().toString(),
										3, "0")).concat(" - ").concat(testCase.servico().getNome());

				if ( !testCase.complemento().isEmpty() )
				{
					complemento = testCase.complemento();
					casoTeste = casoTeste.concat(" ").concat(testCase.complemento());
				}

				tiposPgtos.add(testCase.tipoPgto());

				if ( !tiposPgtos.get(0).equals(EnumTipoPagamento.SEM_CUSTO)
						&& !tiposPgtos.get(0).equals(EnumTipoPagamento.SEM_ONUS) )
				{
	...
	/**
	 * Método responsável por executar instruções para preenchimento dos dados da
	 * baixa
	 * 
	 * @author Evandro Custódio Gonçalves[evandro.custodio@cagece.com.br]
	 *         05/10/2015 - 14:28:40.
	...
	protected void confirmarAdvertencia()
	{
				...
				.findElements(By.id(Atendimento.BTN_SIM_ADVERTENCIA)).size() > 0 )
		{
			...
			DriverUtils.getDriver()
					.findElement(By.id(Atendimento.BTN_SIM_ADVERTENCIA)).click();
						...
						&& DriverUtils
								...
								.findElement(
										By.xpath("//*[@id='tblDados']/table[2]/tbody/tr["
												+ indiceStatusExecucao + "]/td[5]")).getText().trim()
